		...
		this.ajaxpage = function(param){
			...
			this.applyBuildTable = function(){
				...
				function totalPageFun(totalItems,pageSize){
					...
					//var totalPage = parseInt((totalItems + pageSize -1)/pageSize);
		...
		format :function($table){
			...
			for(var i=0;i<subData.length;i++){
				...
				for(var j=0;j<cellVaues.length;j++){
                    if(j==3){
                        trArray.push("<td abbr = "+cellVaues[j]+">");
                        trArray.push(cellVaues[j]+"&nbsp;<a href='javascript:void(0)' onclick='companyinfo(this)' >详情</a>");
                        trArray.push("</td>");
                    }
                    else {
                        trArray.push("<td>");
